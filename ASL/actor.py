...
class Actor:
    def __init__(self, opt):
        ...
        self.explore_frac_scheduler = LinearSchedule(
            opt.decay_step, opt.init_explore_frac, opt.end_explore_frac
        )
        ...
        self.queue = TimeWindowQueue_NTD(
            opt.N, opt.T, opt.state_dim, device=self.A_dvc, padding=0
        )
    ...
    def run(self):
        ...
        while True:
            ...
            if self.total_steps % (self.reset_freq * self.N) == 0:  # Vsteps -> Tsteps
                ...
                print(
                    f"(Actor) {self.exp_name}, Total steps: {round(self.total_steps / 1e3, 2)}k; Obstacle Numbers: {self.envs.O}"
                )
            ...
            """保存数据，更新length, ct, s"""
            self.shared_data.add(
                TW_s, a, r, dw, ct
            )  # 注意ct是用上一次step的， 表示s与s_next是否来自同一条轨迹； (s1,a1,r2,dw2,ct1)
            self.queue.padding_with_done(~ct)  # 根据上一时刻的ct去padding
            ct = ~(
                dw + tr
            )  # 如果当前s_next是”截断状态“或”终止状态“，则s_next与s_next_next是不consistent的，训练时要丢掉
            s = s_next
            ...
            if dw[0] or tr[0]:
                ...
                print(
                    f"(Actor) {self.exp_name}, N{self.N}, Total steps: {round(self.total_steps / 1e3, 2)}k, ep_r: {round(ep_r.item(), 1)}"
                )
                ep_r = 0
                time_consumed = time.time() - self.t_start
                print(
                    f"(Actor) Consumed Time: {round(time_consumed / 3600, 1)}h ({round(time_consumed / 60, 1)}min)"
                )
                print(
                    "-------------------------------------------------------------------------------------------------\n"
                )
            ...
            if self.total_steps > self.explore_steps:
                ...
                if (
                    self.total_steps % (10 * self.N) == 0
                ):  # don't ask shared_data too frequently
    ...
    def fresh_explore_prob(self, steps):
        ...
        ]  # 打乱vectorized e-greedy noise, 让探索覆盖每一个地图
